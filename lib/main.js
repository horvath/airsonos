...
if (flags.get('version')) {
  ...
  sonos.search(function(device, model) {
    ...
    if (model === 'BR100' || model === 'ANVIL' || model === 'ZPS1') return; // ignore Sonos Bridge and Sub device
    device.getZoneAttrs(function(err, zoneAttrs) {
      if (err) throw err;

      var deviceName = zoneAttrs.CurrentZoneName;

      console.log('Setting up Stereo Paired AirSonos for', deviceName, '{' + device.host + ':' + device.port + '}');
